...
TX_THREAD pwm_control;
void pwm_control_create(void);
static void pwm_control_func(ULONG thread_input);
static uint8_t pwm_control_stack[1024] BSP_PLACE_IN_SECTION_V2(".stack.pwm_control") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
void pwm_control_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&pwm_control, (CHAR *) "PWM Thread", pwm_control_func, (ULONG) NULL, &pwm_control_stack,
                            1024, 1, 1, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&pwm_control, 0);
    }
}
